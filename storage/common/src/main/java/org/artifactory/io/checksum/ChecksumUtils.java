...
public abstract class ChecksumUtils {
     ...
     *
     * @param inputStream  The input stream to calculate the checksum value of
    ...
    /**
     * Calculate the checksum of the first <i>partialSize</i> bytes of the input stream.
     * This method does not close the stream.
     *
     * @param inputStream  The input stream to calculate the partial checksum value of. Is not closed by this method.
     * @param checksumType The type of checksum to calculate
     * @return The hexadecimal string of the checksum
     */
    public static String getPartialChecksum(InputStream inputStream, long partialSize, ChecksumType checksumType) {
        Checksum checksum = createFromType(checksumType);
        if (partialSize > 0) {
            try {
                byte[] dataBytes = new byte[CHUNK];
                long bytesLeftToRead = partialSize;
                int length = bytesLeftToRead >= CHUNK ? CHUNK : (int)bytesLeftToRead;
                int nread;
                while ((nread = inputStream.read(dataBytes, 0, length)) != -1) {
                    checksum.update(dataBytes, 0, nread);
                    bytesLeftToRead -= length;
                    if (bytesLeftToRead <= 0) {
                        break;
                    }
                    length = bytesLeftToRead >= CHUNK ? CHUNK : (int)bytesLeftToRead;
                }
            } catch (IOException e) {
                log.error("Could not calculate checksum " + checksumType.name() + " due to: " + e.getMessage(), e);
            }
   ...
   public static Pair<Long, ChecksumsInfo> getSizeAndChecksumsInfo(InputStream in, ChecksumType[] types)
            throws IOException {
