...
public enum ConstantValues {
    ...
    ldapDisablePagingSupportPeriodMinutes("ldap.disable.paging.support.period.minutes", 10),
    ...
    debianReleaseExtendedLabel("debian.release.extended.label", FALSE),
    ...
    gemsCompactIndexEnabled("gems.compact.index.enabled", FALSE),
    gemsCompactIndexWorkers("gems.compact.index.workers", 5),
    gemsCompactEtagStashSize("gems.compact.etag.stash.size", 5000), // #entries per repository
    gemsCompactEtagCacheSize("gems.compact.etag.cache.size", 20000), // For all repos in total
    gemsCompactEtagCacheEvictionTime("gems.compact.etag.cache.eviction.time.minutes", 15),
    gemsCompactPackagesCacheSize("gems.compact.packages.cache.size", 100000),
    gemsCompactPackagesCacheEvictionTime("gems.compact.packages.cache.eviction.time.minutes", 15),
    ...
    nuGetV3DownloadAllowed("nuget.allowV3Download", TRUE),
    haOrphanClusterSingletonLockPeriod("ha.orphan.cluster.singleton.lock.period.minutes", 30),
    ...
    eventBasedMaxPullReplicationInboundEventsPerRepo(
            "replication.eventbased.maxPullReplicationInboundEventsPerRepo", 10_000),
    pullInboundUseSimpleExecutor("replication.eventbased.pullInboundUseSimpleExecutor", false),
    ...
    remoteFetchingListMaximumIteration("remote.fetching.list.maximum.iteration", 3),
    ...
    goLatestVersionSupport("go.latest.version.support", true),
    ...
    listOfQueryParamsIndicateSignUrl("list.of.query.params.indicate.sign.url" , "sig"),
    testingPort("testing.port" , -1),
    listOfReposHostsNeedToAvoidUrlNormalisation("list.of.repos.hosts.need.to.avoid.url.normalisation",
            "mcr.microsoft.com, pkgs.dev.azure.com, nuget.pkg.github.com"),
    ...
    haAffinityTaskFallback("ha.affinity.task.fallback", false),
    haAffinityEnabled("ha.task.affinity.enabled", FALSE),
    oauthUserInfoIdentifier("oauth.user.info.identifier", null),
    oauthRedirectPreferCustomBaseUrl("oauth.redirect.prefer.custom.base.url",
            true), // Should really be false, and should be removed once tested
    defaultPipelinesRepoKey("default.pipelines.repo.key", "jfrogpipelines"),
    eventClientTimeoutSeconds("event.client.timeout.seconds", 60),
    jfEventMaxWorkers("jf.event.max.workers", 60),
    useRepoPathOnRemoteOffline("use.repo.path.on.remote.offline", true)
    ;
