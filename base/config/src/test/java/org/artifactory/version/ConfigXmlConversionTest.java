...
public class ConfigXmlConversionTest /** Don't extend ArtHomeBoundTest */
{
    ...
    public void convert100() throws Exception {
        ...
        assertFalse(cc.getSecurity().isAnonAccessEnabled(), "Annon access should be disabled by default");
    ...
    public void convert179custom() throws Exception {
        ArtifactoryHome.bind(new ArtifactoryHomeStub());
        transform("/config/test/config.1.7.9-external-providers.xml", v179);
        ArtifactoryHome.unbind();
    }

    public void convert211() throws Exception {
        ArtifactoryHome.bind(new ArtifactoryHomeStub());
        CentralConfigDescriptor updated = transform("/config/test/config.2.1.1.w_crowd_wo_allowUsersToAccessProfile.xml", v211);
        assertFalse(updated.getSecurity().getCrowdSettings().isAllowUserToAccessProfile());
        ArtifactoryHome.unbind();
    }

    public void convert227withVerifyAudienceRestriction() throws Exception {
        CentralConfigDescriptor transform =
                transform("/config/test/config.2.2.5_with_saml_settings_enabled.xml", v225);
        assertFalse(transform.getSecurity().getSamlSettings().isVerifyAudienceRestriction());
    }

    public void convert228ForOnlyDockerhubRepos() throws Exception {
        CentralConfigDescriptor transform =
                transform("/config/test/config.2.2.8_docker_retrieval_period.xml", v228);
        assertEquals(transform.getRemoteRepositoriesMap().get("bintray-docker-remote").getRetrievalCachePeriodSecs(),
                7200L);
        assertEquals(transform.getRemoteRepositoriesMap().get("docker-remote-https").getRetrievalCachePeriodSecs(),
                21600L);
        assertEquals(transform.getRemoteRepositoriesMap().get("docker-remote-http").getRetrievalCachePeriodSecs(),
                21600L);
    }
    ...
    public void configVersionSanityCheck() {
        ...
        Set<XmlConverter> allConversions = new HashSet<>();
    ...
    private CentralConfigDescriptor transform(String textXml, ArtifactoryConfigVersion version)
        ...
        String originalXmlString = IOUtils.toString(is, StandardCharsets.UTF_8);
