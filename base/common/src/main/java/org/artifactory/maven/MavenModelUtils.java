...
import static org.jfrog.client.util.PathUtils.ALL_PATH_VALUE;
import static org.jfrog.client.util.PathUtils.trimSlashes;
...
public abstract class MavenModelUtils {
    ...
    private static final DateTimeFormatter UNIQUE_SNAPSHOT_FORMATTER_UPDATED_TIME =
     ...
     * @return Maven unique snapshot version timestamp for the input date. For example: 20160603.113821
     */
     ...
     * @return Maven unique snapshot timestamp for the input date. For example: 20160603.113821
     */
    public static Date uniqueSnapshotTimestampToDate(String dateTime) {
        if (StringUtils.isNotBlank(dateTime) && !dateTime.contains(".")) {
            // TimestampSnapshotComparator might send a timestamp that doesn't have a '.' in it
            return UNIQUE_SNAPSHOT_FORMATTER_UPDATED_TIME.parseDateTime(dateTime).toDate();
        }
    ...
    public static MavenArtifactInfo artifactInfoFromFile(File file) {
        return artifactInfoFromFile(file, file.getName());
    }
    /**
     * Returns a maven artifact info after gathering information from the given file
     *
     * @param file File to gather information from
     * @param originalFileName File name to gather information from
     * @return MavenArtifactInfo object containing gathered information
     */
    public static MavenArtifactInfo artifactInfoFromFile(File file, String originalFileName) {
        ...
        result = getInfoByMatching(originalFileName);
        fillMissingRequiredFields(originalFileName, result);
        return result;
    }
     ...
     *
     * @param originalFileName   Uploaded file name to gather info from
     * @param result MavenArtifactInfo object to append info to
     */
    private static void fillMissingRequiredFields(String originalFileName, MavenArtifactInfo result) {
     ...
     */
    public static String escapeGroupPath(String groupInput) {
        if (StringUtils.isBlank(groupInput)) {
            groupInput = "";
        }
        groupInput = groupInput.replace('\\', '/');
        groupInput = trimSlashes(groupInput);
        groupInput = groupInput.replace('.', '/');
        // wildcard in the beginning means all values/as many folders begin
        if (groupInput.startsWith("*") && !groupInput.startsWith(ALL_PATH_VALUE)) {
            groupInput = "*" + groupInput;
        }
        // Same for the end
        if (groupInput.endsWith("*") && !groupInput.endsWith(ALL_PATH_VALUE)) {
            groupInput = groupInput + "*";
        }
        return groupInput;
    }
