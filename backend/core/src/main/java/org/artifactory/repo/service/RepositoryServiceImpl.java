...
public class RepositoryServiceImpl implements InternalRepositoryService, LicenseEventListener {
    ...
    @Override
    public void saveFilesInternal(@Nonnull List<Pair<RepoPath, File>> files) {
        files.forEach(pair -> {
            try {
                saveFileInternal(pair.getFirst(), new FileInputStream(pair.getSecond())); // No leakage here
            } catch (Exception e) {
                // Log error and continue to the next file
                log.error("Error during saving file {}: {}", pair.getFirst().toPath(), e.getMessage());
            }
        });
    }
    ...
    public void prepareRepoForRemoval(String repoKey) {
        MutableCentralConfigDescriptor configDescriptor = centralConfigService.getMutableDescriptor();
        configDescriptor.setRepoBlackedOut(repoKey, true);
        RemoteRepoDescriptor remoteRepoDescriptor = configDescriptor.getRemoteRepositoriesMap().get(repoKey);
        if (remoteRepoDescriptor != null) {
            remoteRepoDescriptor.setStoreArtifactsLocally(true);
        }
        centralConfigService.saveEditedDescriptorAndReload(configDescriptor);
    }
