...
public class IntegrationCleanerInterceptor extends StorageInterceptorAdapter {
    ...
    public void afterCreate(VfsItem fsItem, MutableStatusHolder statusHolder,
            InterceptorCreateContext ctx) {
        addItemToCleanup(fsItem);
    }

    @Override
    public void afterCopy(VfsItem sourceItem, VfsItem targetItem,
            MutableStatusHolder statusHolder, Properties properties, InterceptorMoveCopyContext ctx) {
        addItemToCleanup(targetItem);
    }

    @Override
    public void afterMove(VfsItem sourceItem, VfsItem targetItem,
            MutableStatusHolder statusHolder, Properties properties, InterceptorMoveCopyContext ctx) {
        addItemToCleanup(targetItem);
    }

    private void addItemToCleanup(VfsItem fsItem) {
        if (fsItem.isFolder()) {
            return;
        }
