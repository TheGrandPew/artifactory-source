...
public interface RemoteRepoInterceptor {
    ...
    /**
     * Allows adding additional logic for expiring an item while attempting to fetch it from the remote repository.
     * see {@link HttpRepo#downloadAndSave(InternalRequestContext, RepoResource)}
     *
     ...
     * @return Ab ExpiryAction which indicates whether an interceptor has declared the resource as expired/not expired
     * or should pass to the "default" Last Modified expiry interceptor implementation.
     * see {@link LastModifiedExpiryRemoteInterceptor#shouldExpireCachedResource}
     */
    default ExpiryAction shouldExpireCachedResource(RepoResource cachedResource, RepoResource remoteResource) {
        return ExpiryAction.DONT_EXPIRE_CONTINUE_NEXT;
    }
}
