...
export class AdminSecurityPermissionsFormController {
    ...
    setAnyRepoOfType(type, isAnyRepoOfThisTypeCheckboxIsChecked) {
        if (isAnyRepoOfThisTypeCheckboxIsChecked) {this.availableRepositories.forEach(repo => {
                if (type === repo.type) {
                    ...
                    if (!_.includes(this.selectedRepositories, repo)) {  // if the repo isn't exist in selected add it
            ...
            this.selectedRepositories.forEach((repo) => {
                if (type === repo.type && !_.includes(this.availableRepositories, repo)) {
    ...
    save() {
        ...
        let getSelectedRepoKeys = () => {
            let anyRepositories = _.pickBy({'ANY LOCAL' : this.anyLocal, 'ANY REMOTE' : this.anyRemote, 'ANY DISTRIBUTION': this.anyDistribution}, _.identity);
