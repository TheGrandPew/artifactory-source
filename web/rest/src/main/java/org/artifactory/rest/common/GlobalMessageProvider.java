...
class GlobalMessageProvider {
    ...
    void decorateWithGlobalMessages(ContainerResponseContext response, AddonsManager addonsManager,
        ...
        if (currentTime - lastUpdateTime > TimeUnit.SECONDS.toMillis(maxCacheAge)) {
            ...
            try {
                if (acquireLock) {
                    ...
                    decorateWithDockerhubCredentialsRecommendation(list, centralConfigService);
    ...
    private void decorateWithDockerhubCredentialsRecommendation(List<FooterMessage> list,
            CentralConfigService centralConfigService) {
        List<RemoteRepoDescriptor> reposWithMissingCredentials = centralConfigService.getDescriptor()
                .getRemoteRepositoriesMap().values().stream()
                .filter(this::isDockerHubRepo)
                .filter(this::isNoUsernameDefined)
                .collect(Collectors.toList());
        if (!reposWithMissingCredentials.isEmpty()) {
            String message = "Please note, it is recommended to set a DockerHub account on your Docker remote " +
                    "repositories to work against Docker Hub. List of relevant repositories: " +
                    Joiner.on(", ").join(reposWithMissingCredentials) + newLine() + " For more information "
                    + dockerRateLimitingPage();
    ...
    private boolean isDockerHubRepo(RepoBaseDescriptor repoDescriptor) {
        return RepoType.Docker.equals(repoDescriptor.getType()) &&
                ((HttpRepoDescriptor) repoDescriptor).getUrl().contains("registry-1.docker.io");
    }

    private boolean isNoUsernameDefined(RemoteRepoDescriptor remoteRepoDescriptor) {
        return StringUtils.isBlank(((HttpRepoDescriptor) remoteRepoDescriptor).getUsername());
    }
        ...
        if (accessService.isAdminUsingOldDefaultPassword()) {
            String message =
                    "<b>Admin Notice</b>: " +
                            ...
                            "It is highly recommended to change the default password. See details " +
                            changeAccessAdminPassword() + ".";
            FooterMessage footerMessage = FooterMessage
                    .createDismissibleError(message, FooterMessage.FooterMessageVisibility.admin);
            list.add(footerMessage);
        }
    ...
    private String psqlIndexMismatchLink() {
        ...
        sb.append(
                "<a href='https://www.jfrog.com/confluence/display/RTF/Troubleshooting#Troubleshooting-RecoveringfromError:AnincompatibleindexhasbeenfoundfortheArtifactory%E2%80%98node_props%E2%80%99databasetable'>")
                .append("click here").append("</a>");
        return sb.toString();
    }
    ...
    private String changeAccessAdminPassword() {
        StringBuilder sb = new StringBuilder();
        sb.append(
                "<a href='https://jfrog.com/knowledge-base/How-to-change-the-default-password-for-access-admin-user' target='_blank'>")
                .append("here").append("</a>");
        return sb.toString();
    }

    private String dockerRateLimitingPage() {
        return "<a href='https://www.jfrog.com/confluence/display/JFROG/Docker+Registry#DockerRegistry" +
                "-RemoteDockerRepositories'>" +
                "click here" + "</a>";
    }
    ...
    private void decorateHeadersWithXrayStatus(List<FooterMessage> list, AddonsManager addonsManager) {
        ...
        if (!xrayAddon.isXrayAlive()) {
            list.add(FooterMessage
                    .createWarning("JFrog Xray is unavailable. For details, please check Artifactory's log files.",
                            admin));
    ...
    private void decorateWithCustomMessage(List<FooterMessage> list) {
        ...
        try {
            messages = JsonUtils.getInstance()
                    .readValueRef(footerMessageRawJson, new TypeReference<List<FooterMessage>>() {
                    });
    ...
    private void addCustomMessageIfNeeded(List<FooterMessage> list, FooterMessage customMessage) {
        ...
        if (customMessage.shouldShowMessage()) {
            //Clear the range prop, no need to output it to ui (and its not ignored since we deserialize the custom
            //messages)
