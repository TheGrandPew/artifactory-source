...
public class GlobalMessageProviderTest extends ArtifactoryHomeBoundTest {
    ...
    private static final FooterMessage ERR = createError(
            "I am putting myself to the fullest possible use, which is all I think that any conscious entity can ever hope to do.",
            FooterMessageVisibility.admin);
    ...
    public void setupMocks() throws IOException {
        ...
        when(centralConfigService.getDescriptor()).thenReturn(new CentralConfigDescriptorImpl());
    ...
    public void setupDefaultMockResponses() {
        ...
        when(configsService.hasConfig(V213ConversionFailFunction.PSQL_NODE_PROPS_INDEX_MISSING_MARKER))
                .thenReturn(false);
    }
    ...
    @Test(dataProvider = "dockerHubMsgProvider")
    public void testDecorateWithNoCredentialsDockerHubFoundRepos(String url, RepoType repoType, String username,
            boolean isAdmin, boolean msgExpected) {
        reset(authService);
        when(authService.isAdmin()).thenReturn(isAdmin);
        when(authService.isAnonymous()).thenReturn(!isAdmin);
        CentralConfigDescriptorImpl centralConfigDescriptor = getDescriptorWithRemoteRepo(url, repoType, username);
        when(centralConfigService.getDescriptor()).thenReturn(centralConfigDescriptor);
        decorateWithGlobalMessages();
        if (msgExpected) {
            assertThat((String) responseMetadata.getFirst(UI_MESSAGES_TAG)).contains(
                    "Please note, it is recommended to set a DockerHub account on your " +
                            "Docker remote repositories to work against Docker Hub. List of relevant repositories:");
        } else {
            assertNoMessages();
        }
        reset(centralConfigService);
        when(centralConfigService.getDescriptor()).thenReturn(new CentralConfigDescriptorImpl());
    }

    private CentralConfigDescriptorImpl getDescriptorWithRemoteRepo(String url, RepoType repoType, String username) {
        CentralConfigDescriptorImpl centralConfigDescriptor = new CentralConfigDescriptorImpl();
        HttpRepoDescriptor dockerRemote = new HttpRepoDescriptor();
        dockerRemote.setUrl(url);
        dockerRemote.setType(repoType);
        dockerRemote.setUsername(username);
        centralConfigDescriptor.addRemoteRepository(dockerRemote);
        return centralConfigDescriptor;
    }

    @DataProvider
    private static Object[][] dockerHubMsgProvider() {
        return new Object[][]{
                //url, repoType, username, isAdminUser, msgExpected
                {"https://registry-1.docker.io/", RepoType.Docker, "", true, true},
                {"http://registry-1.docker.io/", RepoType.Docker, "", true, true},
                {"https://registry-1.docker.io/", RepoType.Docker, "tester", true, false},
                {"https://registry-1.docker.io/", RepoType.Debian, "tester", true, false},
                {"https://registry-1.docker.io/", RepoType.Docker, "", false, false}
        };
    }
    ...
    public void needToChangeAccessAdminPassword() {
        ...
        decorateWithGlobalMessages();
        assertThat((String) responseMetadata.getFirst(UI_MESSAGES_TAG))
                .contains("Your Artifactory instance is set up with " +
    ...
    public void testDismissCode() throws IOException {
        ...
        decorateWithGlobalMessages();
        assertThat((String) responseMetadata.getFirst(UI_MESSAGES_TAG))
                .contains("\"dismissCode\":\"331d7e580da8973a49b2c4565eb7323f\"");
    }
    ...
    private void decorateWithGlobalMessages() {
        msgProvider.decorateWithGlobalMessages(containerRes, addonsManager, storageService, authService, configsService,
                accessService, centralConfigService);
    }
