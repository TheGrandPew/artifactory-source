...
public class RepoConfigModelBuilder {
    ...
    public void populateVirtualRepositoryConfigValuesToModel(VirtualRepoDescriptor descriptor,
        ...
        basic.setResolvedRepositories(
                resolver.getOrderedRepos().stream().map(VirtualSelectedRepository::new).collect(Collectors.toList()));
     ...
     * Populate model configuration from local repository descriptor
     */
    public void populateReleaseBundlesDescriptorValuesToModel(ReleaseBundlesRepoDescriptor descriptor,
            ReleaseBundlesRepositoryConfigModel model) {
        GeneralRepositoryConfigModel general = createGeneralConfig(descriptor);
    ...
    private ReleaseBundlesBasicRepositoryConfigModel createReleaseBundlesBasicConfig(
            ReleaseBundlesRepoDescriptor descriptor) {
    ...
    private ReleaseBundlesAdvancedRepositoryConfigModel createReleaseBundlesAdvancedConfig(
    ...
    private RemoteNetworkRepositoryConfigModel createNetworkConfig(HttpRepoDescriptor descriptor) {
        ...
        if (StringUtils.isNotBlank(clientTleCertificate) &&
                clientTleCertificate.contains(ArtifactWebstartAddon.SSL_CERT_ALIAS_PREFIX)) {
    ...
    TypeSpecificConfigModel createRemoteTypeSpecific(RepoType type, HttpRepoDescriptor descriptor) {
        MavenTypeSpecificConfigModel mavenModel = null;
        TypeSpecificConfigModel model = null;
        switch (type) {
            case Maven:
            ...
            case Docker:
                ...
                dockerType.setRetrievalCachePeriodSecs(descriptor.getRetrievalCachePeriodSecs());
            ...
            case Debian:
                ...
                if (descriptor.getVirtualCacheConfig() != null) {
                    debianTypeSpecificConfigModel.setVirtualRetrievalCachePeriodSecs(descriptor.getVirtualCacheConfig()
                            .getVirtualRetrievalCachePeriodSecs());
                    debianTypeSpecificConfigModel.setVirtualArchitectures(
                            Arrays.asList(descriptor.getDebianDefaultArchitectures().split(",")));
                    debianTypeSpecificConfigModel
                            .setOptionalIndexCompressionFormats(descriptor.getDebianOptionalIndexCompressionFormats());
                }
    ...
    private void populateDistributionValues(DistRepoTypeSpecificConfigModel dist,
        ...
        try {
            BintrayOAuthAppConfigurator appConfigurator = ContextHelper.get()
                    .beanForType(BintrayOAuthAppConfigurator.class);
    ...
    private DownloadRedirectRepoConfigModel getDownloadRedirectConfig(
            DownloadRedirectRepoConfig downloadRedirectRepoConfig) {
